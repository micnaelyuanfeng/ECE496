...
/* 	This is the driver for PCA9685 Chip
...
void init_PCA9685(uint8_t hwAddress){
	...
	writeByte_PCA9685(hwAddress, ALL_LED_ON_L, (uint8_t)0x0000&0xFF);
	writeByte_PCA9685(hwAddress, ALL_LED_ON_H, (uint8_t)0x0000>>8);
	writeByte_PCA9685(hwAddress, ALL_LED_OFF_L, (uint8_t)0x0000&0xFF);
	writeByte_PCA9685(hwAddress, ALL_LED_OFF_H, (uint8_t)0x0000>>8);
...
void set_PWM_frequency_PCA9685(uint8_t hwAddress, uint16_t pwmFrequency){
	...
	float prescaleFreqFloat = CPUCLK / PRESCALELEVEL;
	prescaleFreqFloat = prescaleFreqFloat / (float)pwmFrequency;
	prescaleFreqFloat = prescaleFreqFloat - 1;
...
void set_PWM_PCA9685(uint8_t hwAddress, uint8_t pwmChannel, uint16_t onTime, uint16_t offTime){
	...
	writeByte_PCA9685(hwAddress, LED0_ON_L+4*pwmChannel, onTime & 0xFF);
	writeByte_PCA9685(hwAddress, LED0_ON_H+4*pwmChannel, onTime >> 8);
	writeByte_PCA9685(hwAddress, LED0_OFF_L+4*pwmChannel, offTime & 0xFF);
	writeByte_PCA9685(hwAddress, LED0_OFF_H+4*pwmChannel, offTime >> 8);	
}
...
void writeByte_PCA9685(uint8_t hwAddress, uint8_t writeAddress, uint8_t writeData){
	...
	BSC0_A = hwAddress;
	// Specify the size of data (1 byte)
	BSC0_DLEN = 1;
	// Write to peripheral in (address, data) order
	BSC0_FIFO = writeAddress;
	BSC0_FIFO = writeData;
